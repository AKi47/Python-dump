...
def MainAlgorithm(Display):
    ...
    #running,mouse_pos,click1 = EventGet()   
    Algorithm(Display,coords_dict,node_queue,adjacency_list,distances,path) #Reset
    #pygame.display.flip()


def Algorithm(Display,coords_dict,node_queue,adjacency_list,distances,path):
    current_node = ""
    adjacent_nodes = []
    comparison = 0
    while True:
...
def SetAdjacencyList(coords_dict,node_queue):
    ...
    for Coordinate in node_queue:
        ...
        if ord(Coordinate[0]) == 97:
            above = chr(ord(Coordinate[0])-7) + str(Coordinate[1])
        else:
            above = chr(ord(Coordinate[0])-1) + str(Coordinate[1])

        if ord(Coordinate[0]) == 90:
            below = chr(ord(Coordinate[0])+7) + str(Coordinate[1])
        else:
            below = chr(ord(Coordinate[0])+1) + str(Coordinate[1])
        
        if ord(Coordinate[1]) == 90:
            right = str(Coordinate[0]) + chr(ord(Coordinate[1])+7)
        else:
            right = str(Coordinate[0]) + chr(ord(Coordinate[1])+1)
        if ord(Coordinate[0]) == 97:
            left = str(Coordinate[0]) + chr(ord(Coordinate[1])-7)
        else:
...
def Start():
    ...
    if choice == "1":
        while True:
            ...
            b_width = int(input("How many boxes wide (Max 26):"))
            b_height = int(input("How many boxes high (Max 26):"))
            if b_width <= 26 and b_height <= 26:
                break
